...
playFX=function(type,label){
 ...
 const tale=/^tale/.test(type),variant=label==='Murmures dissonants'?'whisper':label==='Inspiration bardique'?'inspiration':tale?'tale-prototype':'',w=Math.max(innerWidth,700),h=Math.max(innerHeight,700);
 ...
 if(variant==='whisper'){
  ...
  addWisp(15);for(let i=0;i<8;i++){const p=document.createElement('i');p.className='particle spirit-shape';p.style.cssText=`left:${8+Math.random()*84}%;top:${87+Math.random()*17}%;--w:${60+Math.random()*75}px;--h:${150+Math.random()*150}px;--blur:${4+Math.random()*7}px;--x:${(-.08+Math.random()*.16)*w}px;--y:${-(.48+Math.random()*.42)*h}px;--d:${2.05+Math.random()*.85}s;--delay:${Math.random()*.42}s;--alpha:${.16+Math.random()*.15}`;box.appendChild(p)}
 }else{
